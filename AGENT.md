...
Post-M5 distribution work (done): the Tauri **Rust shell** now **compiles** (`cargo
build`, tauri 2/wry/webview2); real app icons are generated by `scripts/make_icons.py`;
and the §8 packaging question is resolved — `scripts/build_kernel.py` produces a
PyInstaller `dist/agent-kernel.exe` that boots and serves the API + bundled UI with no
system Python (verified), and the shell auto-detects a bundled kernel binary next to it.

Standing gaps (need external resources, not code): the native Tauri **window** can't be
*displayed*/screenshotted headlessly (needs a desktop session — `npm run tauri dev`);
a live **OpenAI** run needs `OPENAI_API_KEY` (paid — don't spend without asking); a live
**Ollama** run needs Ollama installed + a pulled model. All three adapters/paths are
unit-tested, and the OpenAI-compatible path is proven live via LM Studio.

**No further milestones.** Remaining is optional hardening/distribution, not new scope.
